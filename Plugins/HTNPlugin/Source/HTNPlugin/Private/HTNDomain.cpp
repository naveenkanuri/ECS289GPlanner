...
bool FSHTNDomain::Validate(bool bDebug)
{
	...
	if (!IsCompositeTask(RootTaskName))
	{
		...
		if (bDebug)
		{
			UE_LOG(HTNPlugin, Error, TEXT("The root task %s is not a composite task"), *RootTaskName.ToString());
		}
	}
			...
			for (const FName& Task : Method.Tasks)
			{
				if (FirstTask)
				{
					...
					if (CompositeTask.Type == ESHTNCompositeType::Scored)
					{
						...
						{
							bValid = false;
							UE_LOG(HTNPlugin, Error, TEXT("Task %s is expected to be primitive due to owning Composite being of type 'Scored'"), *Task.ToString());
				...
				if (!(IsPrimitiveTask(Task) || IsCompositeTask(Task)))
				{
					...
					if (bDebug)
					{
						UE_LOG(HTNPlugin, Error, TEXT("Task %s does not exist in the network"), *Task.ToString());
					}
				}
			}
		}
...
bool FSHTNWorldState::CheckCondition(const FSHTNCondition & Condition) const
{
	...
	default:
		UE_LOG(HTNPlugin, Warning, TEXT("Default case hit in CheckCondition"));
		return false;
	}
}
...
void FSHTNWorldState::ApplyEffect(const FSHTNEffect & Effect)
{
	...
	default:
		UE_LOG(HTNPlugin, Warning, TEXT("Default case hit in ApplyEffect"));
		return;
	}
}
