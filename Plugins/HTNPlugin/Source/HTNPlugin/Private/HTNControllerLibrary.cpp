...
bool USHTNControllerLibrary::RunHTNPlanner(AAIController* AIController, TSubclassOf<USHTNNetwork_BlueprintBase> HTNNetwork)
{
	...
	{
		UE_LOG(HTNPlugin, Error, TEXT("RunHTNPlanner: Can't run the planner on a NULL AIController"));
		return false;
	}
	else if (HTNNetwork.GetDefaultObject() == NULL)
	{
		UE_LOG(HTNPlugin, Error, TEXT("RunHTNPlanner: Can't run the planner on a NULL Network"));
		return false;
	}
	else if (HTNNetwork.GetDefaultObject()->WorldStateAsset == NULL)
	{
		UE_LOG(HTNPlugin, Error, TEXT("RunHTNPlanner: Can't run the planner with a NULL WorldStateAsset"));
		return false;
	}
	...
	if (bSuccess)
	{
		...
		if (SHTNComp == NULL)
		{
			UE_LOG(HTNPlugin, Log, TEXT("RunHTNPlanner: Spawning new SHTNComponent..."));
...
USHTNComponent* USHTNControllerLibrary::GetSHTNComponent(AActor * Actor)
{
	...
	if (Controller == nullptr)
	{
		UE_LOG(HTNPlugin, Error, TEXT("%s is not an actor that has an AIController"), *Actor->GetActorLabel());
		return nullptr;
	}
	...
	if (SHTNComp == nullptr)
	{
		UE_LOG(HTNPlugin, Error, TEXT("%s is not a controller with an SHTNComponent as BrainComponent"), *Controller->GetActorLabel());
		return nullptr;
	}
