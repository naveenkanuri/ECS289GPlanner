...
void USHTNComponent::StartLogic()
{
	if (bIsRunning)
	{
		UE_LOG(HTNPlugin, Warning, TEXT("%s already has a running planner. Skipping logic.."), *GetName());
		return;
	}
...
void USHTNComponent::StopLogic(const FString & Reason)
{
	if (!bIsRunning)
	{
		UE_LOG(HTNPlugin, Warning, TEXT("%s doesnt have a running planner. Skipping logic.."), *GetName());
		return;
	}
...
{
	UE_LOG(HTNPlugin, Warning, TEXT("Execution updates: PAUSED (%s)"), *Reason);
	bIsPaused = true;
}
...
void USHTNComponent::GeneratePlan()
{
	if (Domain.IsValid())
	{
		...
		{
			UE_LOG(HTNPlugin, Log, TEXT("Plan result for: %s with %i cycles"), *AIOwner->GetPawn()->GetActorLabel(), Plan.Cycles);
			UE_LOG(HTNPlugin, Log, TEXT("----------------------------------------"));
			for (int32 TaskIndex = 0; TaskIndex < Plan.TaskNames.Num(); ++TaskIndex)
			{
				UE_LOG(HTNPlugin, Log, TEXT("%d : %s"), TaskIndex, *Plan.TaskNames[TaskIndex].ToString());
			}
			UE_LOG(HTNPlugin, Log, TEXT("----------------------------------------"));
		}
		else
		{
			UE_LOG(HTNPlugin, Error, TEXT("Failed to find plan for: %s"), *AIOwner->GetPawn()->GetActorLabel());
	...
	{
		UE_LOG(HTNPlugin, Error, TEXT("Invalid Domain for: %s"), *AIOwner->GetPawn()->GetActorLabel());
	}
}
...
void USHTNComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction * ThisTickFunction)
{
	...
	if (CurrentOperator == nullptr)
	{
		if (Plan.TaskSequence.Num() > 0)
		{
			...
			if (!CurrentOperator->CheckCondition(BlackboardState, (uint8)CurrentTask.Parameter))
			{
				bReplan = true;
				CurrentOperator = nullptr;
				UE_LOG(HTNPlugin, Log, TEXT("%s is replanning due failed condition at runtime..."), *AIOwner->GetPawn()->GetActorLabel());
				return;
			}
		}
		else
		{
			...
			bReplan = true;
			UE_LOG(HTNPlugin, Log, TEXT("%s is replanning due to reaching end of plan..."), *AIOwner->GetPawn()->GetActorLabel());
			return;
		}
	}
	...
	if (CurrentOperator)
	{
		...
		else if(CurrentOperator->GetResult() == ESHTNOperatorResult::Failed)
		{
			...
			UE_LOG(HTNPlugin, Log, TEXT("%s is replanning due to failed operator result of: %s with parameter %i"), *AIOwner->GetPawn()->GetActorLabel(), *CurrentOperator->CDOName.ToString(), CurrentTask.Parameter);
		}
